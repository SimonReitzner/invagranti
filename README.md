...
|---------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|---------------------|----------------------------------|---------------------------------------------------------|
| `vagrant.global.box`               | Specifies the Vagrant box to use for all VMs unless overridden at the VM level.                                                                    | String              | `'ubuntu/bionic64'`              | `'ubuntu/focal64'`                                      |
...
| `vagrant.global.provider.virtualbox.memory` | Globally defines the memory setting for all VMs in the YAML file unless overridden at the VM level.                                                | String or Integer   | `'1024'`                         | `'2048'`                                                |
| `vagrant.global.provider.virtualbox.cpus`   | Globally defines the number of CPU cores for all VMs in the YAML file unless overridden at the VM level.                                            | Integer             | `2`                              | `4`                                                     |
| `vagrant.global.provider.virtualbox.gui`    | Globally specifies whether all VMs should start with a GUI unless overridden at the VM level.                                                      | Boolean             | `false`                          | `true`                                                  |
| `vagrant.global.network.forwarded_port`     | Globally specifies the forwarded port configuration for all VMs unless overridden at the VM level.                                                 | Hash (guest, host)  | `{ guest: 80, host: 8080 }`      | `{ guest: 443, host: 8443 }`                            |
| `vagrant.global.network.private_network`    | Globally specifies the private network configuration for all VMs unless overridden at the VM level.                                                | Hash (type, ip)     | `{ type: 'dhcp' }`               | `{ type: 'static', ip: '192.168.33.10' }`               |
...
| `vagrant.global`                   | Defines global settings that apply to all VMs, unless overridden at the individual VM level.                                                       | Hash                | `nil`                            | `{ box: 'ubuntu/bionic64', provider: {...} }`           |

#### Descriptions

1. **`vagrant.global.box`**: Defines a global box to be used by all VMs unless overridden for specific VMs.
...
14. **`vagrant.global.provider.virtualbox.memory`**: Global setting for memory assigned to all VMs, unless individually overridden.
15. **`vagrant.global.provider.virtualbox.cpus`**: Global setting for CPU cores assigned to all VMs, unless individually overridden.
16. **`vagrant.global.provider.virtualbox.gui`**: Global setting to enable or disable the GUI for all VMs, unless overridden.
17. **`vagrant.global.network.forwarded_port`**: Global port forwarding settings, applied to all VMs unless overridden.
18. **`vagrant.global.network.private_network`**: Global private network configuration for all VMs, unless overridden.
19. **`vagrant.vms[]`**: Contains all individual VM configurations, with each VM’s unique settings.
20. **`vagrant.global`**: Contains global configurations for all VMs, such as box, network, and provider settings.
